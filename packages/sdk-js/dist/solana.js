...
async function getSolActor() {
    if (!solActor) {
        if (!CANISTER_ID)
            throw new Error('SOLANA_SIGNER_CANISTER_ID not configured');
        const ag = await getAgent();
        solActor = Actor.createActor(solanaIDL, { agent: ag, canisterId: CANISTER_ID });
    }
    return solActor;
}
export async function getSolAddress() {
...
export async function requestAirdrop(address, lamports) {
    const a = await getSolActor();
    return a.request_airdrop(address, lamports);
}
export async function transferSol(to, lamports) {
    const a = await getSolActor();
    return a.build_and_send_transfer(to, lamports);
}
export async function getSolTx(signature) {
    const a = await getSolActor();
...
export async function sendRawTxBase64(base64) {
    const a = await getSolActor();
