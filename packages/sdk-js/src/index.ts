...
let evmRpcActor: any = null;

async function getEVMRpcActor() {
  if (!evmRpcActor) {
    const agentInstance = await getAgent();
    const evmRpcIDL = ({ IDL }: any) => {
...
export async function mintIQuBeOnICP(dataHash: string): Promise<{ receiptId: string }> {
  const receiptId = await callICPCanister('proof_of_state', 'issue_receipt', [dataHash]);
  if (!receiptId) throw new Error('Failed to mint on ICP');
  return { receiptId };
}

// Anchor batches now: batch + anchor
export async function anchorBatchesNow(): Promise<{ batchRoot?: string; anchorTxId?: string | null }> {
  const batchRoot = await callICPCanister('proof_of_state', 'batch', []);
...
export async function getBitcoinAddress(derivationPath: number[][] = []): Promise<{ address: string; public_key?: number[] }> {
...
export async function submitCrossChainMessage(sourceChainId: number, destChainId: number, payload: string): Promise<string> {
  ...
  if (!messageId) throw new Error('Failed to submit DVN message');
  return messageId;
}

export async function attestDVNMessage(messageId: string, validators: string[] = ['v1','v2']): Promise<void> {
...
export async function getCrossChainMessageStatus(messageId: string): Promise<{ attestations: number; ready: boolean }> {
  const att = await callICPCanister('cross_chain_service', 'get_message_attestations', [messageId]);
  const readyMessages = await callICPCanister('cross_chain_service', 'get_ready_messages', []);
  const ready = Array.isArray(readyMessages) && readyMessages.some((m: any) => m.id === messageId);
  return { attestations: Array.isArray(att) ? att.length : 0, ready };
}

// Burn state wrappers (proof_of_state)
export async function setBurnState(receiptId: string, messageId: string, burned: boolean): Promise<string | null> {
  try {
...
export async function getBurnState(receiptId: string): Promise<any | null> {
  try {
...
// DVN list helpers
export async function getPendingDVNMessages(): Promise<any[]> {
  try {
...
export async function getReadyDVNMessages(): Promise<any[]> {
  try {
...
export async function createAnchorTransaction(dataRoot: string, utxos: any[], amountSats: bigint): Promise<any> {
...
export async function signTransaction(unsignedTx: any, derivationPath: number[][] = []): Promise<any> {
...
export async function fetchTestnetUtxos(address: string): Promise<TestnetUtxo[]> {
  const url = `https://blockstream.info/testnet/api/address/${address}/utxo`;
  const r = await fetch(url);
  if (!r.ok) throw new Error(`Blockstream UTXO fetch failed: ${r.status}`);
  ...
  const details = await Promise.all(
    (data as any[]).map(async (u) => {
      ...
      if (!tx.ok) throw new Error(`Blockstream tx fetch failed: ${tx.status}`);
      const txJson = await tx.json();
      const vout = txJson.vout?.[u.vout];
      const scriptpubkey = vout?.scriptpubkey as string;
      return { txid: u.txid, vout: u.vout, value: u.value, scriptpubkey } as TestnetUtxo;
    })
  );
  return details;
}
...
export function toCanisterUtxos(utxos: TestnetUtxo[]): any[] {
  return utxos.map((u) => ({
    txid: u.txid,
    vout: u.vout,
...
export async function createSignBroadcastAnchor(
  dataRoot: string,
  utxos: TestnetUtxo[],
  amountSats: bigint,
